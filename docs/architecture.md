...
**Version:** 1.2.0
**Date:** 2026-09-01 (second passage de nettoyage — voir `CHANGELOG_CLEANUP.md`)
...
| - Lags (1-336h)     | | - Lags (1-336h)     |                  |
| - Rolling windows   | | - Rolling windows   |                  |
| - HDD/CDD           | | - HDD/CDD           |                  |
| - Cycliques         | | - Cycliques horizon |                  |
| - Holidays          | | - Holidays          |                  |
+--------------------+ +--------------------+                  |
        |                        |                               |
        v                        v                               |
...
+--------------+       +------------------+                      |
        |                        |                                |
        v (récursif)             v (direct, sans récursion)       |
+--------------------+  +----------------------+   <--------------+
...
**`ml_features_gold_24h`** — features + target pour le modèle 24h
(`target_demand_mw` = demande à `target_datetime`, `forecast_horizon_hours`
toujours égal à 24). Une vue **`ml_features_training_gold`** filtre les
lignes valides (`has_valid_target = 1`) pour l'entraînement.

**`ml_features_gold_7j`** — features multi-horizon pour le modèle 7j : une
ligne par `(zone, issue_datetime, forecast_horizon_hours)`, avec
`forecast_horizon_hours` de 1 à 168 et ses transformations cycliques
(`forecast_horizon_sin_24h`, `_cos_24h`, `_sin_168h`, `_cos_168h`) en feature.

**`load_forecast_gold`** — prédictions batch du modèle 24h
```sql
...
**`load_forecast_7j`** — prédictions batch du modèle 7j, tous horizons
(H+1 à H+168) écrits en une seule passe par `09b_batch_prediction_7j_direct.py`
(même structure de colonnes que `load_forecast_gold`, plus
`forecast_horizon_hours`).

**`load_shap_gold`** / **`load_shap_7j`** — contributions SHAP par feature
pour chaque prédiction, respectivement pour le modèle 24h et le modèle 7j.
...
LightGBM entraîné sur `ml_features_gold_24h`, tag `forecast_horizon: "24h"`,
...
LightGBM **multi-horizon direct**, entraîné sur `ml_features_gold_7j` :
`forecast_horizon_hours` (1 à 168) est une feature du modèle (avec
transformations sqrt/log1p/sin/cos), donc un seul modèle prédit n'importe quel
horizon dans [1h, 168h] en une seule inférence (pas de récursion). Enregistré
sous `ontario_demand_lightgbm_7j`.

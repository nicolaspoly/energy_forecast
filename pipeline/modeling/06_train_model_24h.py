...
# DBTITLE 1,Title
# MAGIC %md
# MAGIC # 08 — Entraînement LightGBM direct multi-horizon 24h
# MAGIC
# MAGIC Ce notebook entraîne un modèle direct, non récursif, pour prévoir
# MAGIC les 24 prochaines heures de demande électrique par zone.
# MAGIC
...
# MAGIC - **Optuna** pour l'optimisation des hyperparamètres
# MAGIC
...
ARTIFACT_DIRECTORY = (
    "/tmp/lightgbm_energy_forecast_24h"
)

print("=" * 80)
print("ENTRAÎNEMENT LIGHTGBM MULTI-HORIZON 24h")
print("=" * 80)
...
# MAGIC %md
# MAGIC ## 3. Conversion Pandas et sous-échantillonnage

# COMMAND ----------

# DBTITLE 1,Sous-échantillonnage : 1 émission/jour par zone
# Un modèle 24h émet une prévision par jour.
# Garder 1 émission/jour/zone (stride=1) pour le modèle 24h.
# Le volume est gérable en Pandas : ~24 horizons × 10 zones × ~2000 émissions ≈ 480K lignes.
from pyspark.sql import functions as F

_SAMPLING_STRIDE_DAYS = 1  # garder 1 jour sur 1 (toutes les émissions quotidiennes)
...
print(
    ...
    f"(1 émission/{_SAMPLING_STRIDE_DAYS}j/zone, {MAX_FORECAST_HORIZON} horizons) : "
...
# MAGIC %md
# MAGIC ## 4. Contrôle de la couverture H+1 à H+24
...
complete_coverage_mask = (
    coverage["horizon_count"].eq(MAX_FORECAST_HORIZON)
    & coverage["minimum_horizon"].eq(MIN_FORECAST_HORIZON)
    & coverage["maximum_horizon"].eq(MAX_FORECAST_HORIZON)
)
...
print(
    f"Couverture complète H+{MIN_FORECAST_HORIZON} à H+{MAX_FORECAST_HORIZON} : "
    f"{coverage_rate:.2%}"
)
...
if pd_data.empty:
    raise ValueError(
        ...
        f"les {MAX_FORECAST_HORIZON} horizons complets."
    )
...
def calculate_metrics(y_true, y_pred):
    ...
    if len(y_true) == 0:
        return {"mae": np.nan, "rmse": np.nan, "r2": np.nan, "mape": np.nan, "smape": np.nan, "wape": np.nan, "bias": np.nan}
    mae = mean_absolute_error(y_true, y_pred)
    rmse = np.sqrt(mean_squared_error(y_true, y_pred))
    r2 = r2_score(y_true, y_pred) if len(y_true) >= 2 else np.nan
    non_zero_mask = np.abs(y_true) > 1e-6
    mape = np.mean(np.abs((y_true[non_zero_mask] - y_pred[non_zero_mask]) / y_true[non_zero_mask])) * 100.0 if non_zero_mask.any() else np.nan
    smape_denominator = np.abs(y_true) + np.abs(y_pred)
    smape_mask = smape_denominator > 1e-6
    smape = np.mean(2.0 * np.abs(y_true[smape_mask] - y_pred[smape_mask]) / smape_denominator[smape_mask]) * 100.0 if smape_mask.any() else np.nan
    target_sum = np.abs(y_true).sum()
    wape = np.abs(y_true - y_pred).sum() / target_sum * 100.0 if target_sum > 1e-6 else np.nan
    bias = np.mean(y_pred - y_true)
    return {"mae": float(mae), "rmse": float(rmse), "r2": float(r2), "mape": float(mape), "smape": float(smape), "wape": float(wape), "bias": float(bias)}
...
def train_lightgbm(train_frame, validation_frame, features, target_column, seed=42):
    ...
    categorical_features = [f for f in features if str(X_train[f].dtype) == "category"]
    model = lgb.LGBMRegressor(
        ...
        reg_alpha=0.10, reg_lambda=1.00, random_state=seed,
        n_jobs=-1, importance_type="gain", verbosity=-1,
    )
    model.fit(
        X_train, y_train,
...
def get_feature_importance(model, features):
    ...
    importance["gain_pct"] = importance["gain"] / total_gain * 100.0 if total_gain > 0 else 0.0
    importance = importance.sort_values("gain", ascending=False).reset_index(drop=True)
    importance["cumulative_gain_pct"] = importance["gain_pct"].cumsum()
    return importance

def calculate_grouped_metrics(source_data, predictions, group_columns):
    """Calcule les métriques par groupes."""
    result = source_data[group_columns + [TARGET_COLUMN]].copy()
    result["prediction_mw"] = predictions
    rows = []
    grouped = result.groupby(group_columns, observed=True)
    for group_key, group_data in grouped:
        if not isinstance(group_key, tuple):
            group_key = (group_key,)
        row = {column: (str(value) if column == ZONE_COLUMN else int(value) if column in {"forecast_day", HORIZON_COLUMN} else value) for column, value in zip(group_columns, group_key)}
        metrics = calculate_metrics(group_data[TARGET_COLUMN], group_data["prediction_mw"])
        row.update({"observations": len(group_data), **metrics})
        rows.append(row)
...
def get_selected_features(feature_count):
    """Sélectionne les meilleures features en conservant toutes les variables obligatoires."""
    selected = ranked_features[:feature_count].copy()
    for feature in MANDATORY_FEATURES:
        if feature in baseline_features and feature not in selected:
...
print(f"Configurations testées : {feature_counts}")

for selection_key in feature_counts:
    current_features = get_selected_features(selection_key)
    ...
    selection_results.append({
        ...
        "requested_feature_count": selection_key,
        "actual_feature_count": len(current_features),
        "best_iteration": int(current_model.best_iteration_),
        **current_metrics,
    })
    selection_models[selection_key] = {"model": current_model, "features": current_features}
    print(
...
selection_results_df = (
    ...
    .sort_values(["actual_feature_count", "wape"])
    .reset_index(drop=True)
)
...
best_validation_wape = selection_results_df["wape"].min()

acceptable_results = (
    selection_results_df[
        selection_results_df["wape"]
        <= (best_validation_wape + WAPE_TOLERANCE_PERCENTAGE_POINT)
    ].copy()
)

chosen_result = (
    acceptable_results
    .sort_values(["actual_feature_count", "wape", "mae"])
    .iloc[0]
)

CHOSEN_SELECTION_KEY = int(chosen_result["selection_key"])
selected_features = selection_models[CHOSEN_SELECTION_KEY]["features"]
selected_validation_model = selection_models[CHOSEN_SELECTION_KEY]["model"]
BEST_ITERATION = int(selected_validation_model.best_iteration_)

if HORIZON_COLUMN not in selected_features:
    raise ValueError("forecast_horizon_hours doit rester dans les features du modèle.")

print("=" * 80)
print("CONFIGURATION SÉLECTIONNÉE")
print("=" * 80)
print(f"Meilleur WAPE validation : {best_validation_wape:.3f} %")
print(f"WAPE retenu              : {chosen_result['wape']:.3f} %")
print(f"Features sélectionnées   : {len(selected_features)}")
print(f"Nombre d'arbres          : {BEST_ITERATION}")
...
# MAGIC
# MAGIC Early stopping (30 rounds) pour trouver automatiquement le nombre optimal d'arbres.
...
def optuna_objective(trial):
    ...
    model = lgb.LGBMRegressor(
        objective="regression_l1", n_estimators=300, **params,
        subsample_freq=1, random_state=RANDOM_STATE, n_jobs=-1, verbosity=-1,
    )
    model.fit(
...
default_params = {
    ...
    "min_child_samples": 100, "subsample": 0.80, "colsample_bytree": 0.80,
...
train_validation_data = (
    pd.concat([train_data, validation_data], axis=0)
    .sort_values([ISSUE_DATETIME_COLUMN, ZONE_COLUMN, HORIZON_COLUMN])
    .reset_index(drop=True)
)

X_train_validation = train_validation_data[selected_features].copy()
y_train_validation = train_validation_data[TARGET_COLUMN].copy()
...
categorical_features_final = [
    ...
    if str(X_train_validation[feature].dtype) == "category"
]
...
print(f"Réentraînement avec {len(selected_features)} features...")
...
train_validation_predictions = final_model.predict(X_train_validation)
test_predictions = final_model.predict(X_test)

train_validation_metrics = calculate_metrics(y_train_validation, train_validation_predictions)
test_metrics = calculate_metrics(y_test, test_predictions)

print_metrics("TRAIN + VALIDATION", train_validation_metrics)
print_metrics("TEST FINAL", test_metrics)

test_predictions_df = test_data[
    [ISSUE_DATETIME_COLUMN, TARGET_DATETIME_COLUMN, HORIZON_COLUMN, "forecast_day", ZONE_COLUMN, TARGET_COLUMN]
].copy()

test_predictions_df["prediction_mw"] = test_predictions
test_predictions_df["error_mw"] = test_predictions_df["prediction_mw"] - test_predictions_df[TARGET_COLUMN]
test_predictions_df["absolute_error_mw"] = np.abs(test_predictions_df["error_mw"])
test_predictions_df["absolute_percentage_error"] = np.where(
    np.abs(test_predictions_df[TARGET_COLUMN]) > 1e-6,
    test_predictions_df["absolute_error_mw"] / np.abs(test_predictions_df[TARGET_COLUMN]) * 100.0,
    np.nan,
)
...
# MAGIC des intervalles de prévision sur les 24 horizons :
...
metrics_by_zone_and_day = (
    calculate_grouped_metrics(
        source_data=test_data,
        predictions=test_predictions,
        group_columns=[ZONE_COLUMN, "forecast_day"],
    ).sort_values([ZONE_COLUMN, "forecast_day"])
)
...
zonal_pred = test_predictions_df[test_predictions_df[ZONE_COLUMN].isin(individual_zones)]
...
if "Ontario" in all_zones:
    ...
    coherence_gap = np.mean(np.abs(comparison["bottomup_actual_mw"] - comparison["direct_actual_mw"]))
    print(f"\nÉcart moyen actual bottom-up vs Ontario direct : {coherence_gap:.1f} MW")
    ontario_metrics = calculate_metrics(comparison["bottomup_actual_mw"], comparison["bottomup_predicted_mw"])
    ontario_direct_metrics = calculate_metrics(comparison["direct_actual_mw"], comparison["direct_predicted_mw"])
else:
    ontario_metrics = calculate_metrics(ontario_bottomup["bottomup_actual_mw"], ontario_bottomup["bottomup_predicted_mw"])
...
if "Ontario" in all_zones:
    ...
    print(f"  du modèle direct (Ontario prédit séparérement).")
...
latest_day_ontario = ontario_bottomup[
    ontario_bottomup[ISSUE_DATETIME_COLUMN] == latest_test_issue
].sort_values(TARGET_DATETIME_COLUMN)

fig, ax = plt.subplots(figsize=(16, 7))
ax.fill_between(
    latest_day_ontario[TARGET_DATETIME_COLUMN],
    latest_day_ontario["bottomup_p10_mw"],
    latest_day_ontario["bottomup_p90_mw"],
    alpha=0.25, color="orange", label="Intervalle P10-P90",
)
ax.plot(latest_day_ontario[TARGET_DATETIME_COLUMN], latest_day_ontario["bottomup_actual_mw"], label="Demande réelle Ontario")
ax.plot(latest_day_ontario[TARGET_DATETIME_COLUMN], latest_day_ontario["bottomup_predicted_mw"], label="Prédiction (bottom-up)")
...
for baseline_feature in ["demand_lag_24h", "demand_lag_168h"]:
    if baseline_feature not in test_data.columns:
        continue
    baseline_predictions = pd.to_numeric(test_data[baseline_feature], errors="coerce").to_numpy()
    baseline_metrics = calculate_metrics(test_data[TARGET_COLUMN], baseline_predictions)
    baseline_rows.append({"model": baseline_feature, **baseline_metrics})

baseline_rows.append({
    "model": "lightgbm_direct_24h",
    **test_metrics,
})
...
final_importance = get_feature_importance(final_model, selected_features)
...
ax.set_title("Top 30 des features du modèle H+1 à H+24")
...
axes[0].plot(
    metrics_by_horizon[HORIZON_COLUMN],
    metrics_by_horizon["mae"],
)
axes[0].set_ylabel("MAE (MW)")
axes[0].set_title("Erreur selon l'horizon")
...
axes[1].set_xlabel("Horizon de prévision (heures)")
axes[1].set_ylabel("Biais moyen (MW)")
axes[1].grid(True, alpha=0.3)

for axis in axes:
    for boundary in range(6, 24, 6):
        axis.axvline(boundary, color="grey", linestyle=":", alpha=0.6)
...
# MAGIC %md
# MAGIC ## 20. Visualisation de la dernière prévision quotidienne du test

# COMMAND ----------

latest_test_issue = test_predictions_df[ISSUE_DATETIME_COLUMN].max()

latest_day = test_predictions_df[
    test_predictions_df[ISSUE_DATETIME_COLUMN] == latest_test_issue
].copy()

latest_day_global = (
    latest_day
    .groupby(TARGET_DATETIME_COLUMN, as_index=False)
    .agg(
...
ax.plot(latest_day_global[TARGET_DATETIME_COLUMN], latest_day_global["actual_demand_mw"], label="Demande réelle")
ax.plot(latest_day_global[TARGET_DATETIME_COLUMN], latest_day_global["predicted_demand_mw"], label="Demande prédite")

ax.set_xlabel("Date et heure cible")
ax.set_ylabel("Demande totale (MW)")
ax.set_title("Dernière prévision quotidienne du jeu de test")
ax.legend()
ax.grid(True, alpha=0.3)
...
artifact_dataframes = {
    "baseline_feature_importance.csv": baseline_importance,
    "final_feature_importance.csv": final_importance,
    "feature_selection_results.csv": selection_results_df,
    "test_metrics_by_zone.csv": metrics_by_zone,
    "test_metrics_by_forecast_day.csv": metrics_by_forecast_day,
    "test_metrics_by_zone_and_day.csv": metrics_by_zone_and_day,
    "test_metrics_by_horizon.csv": metrics_by_horizon,
    "baseline_comparison.csv": baseline_comparison,
    "test_predictions.csv": test_predictions_df,
    "walk_forward_results.csv": wf_results_df,
    "walk_forward_predictions.csv": wf_predictions_all_df,
}

for filename, dataframe in artifact_dataframes.items():
    save_dataframe_csv(dataframe, os.path.join(ARTIFACT_DIRECTORY, filename))

selected_features_path = os.path.join(ARTIFACT_DIRECTORY, "selected_features.json")
with open(selected_features_path, "w", encoding="utf-8") as file:
    json.dump(selected_features, file, ensure_ascii=False, indent=2)

zone_categories_path = os.path.join(ARTIFACT_DIRECTORY, "zone_categories.json")
with open(zone_categories_path, "w", encoding="utf-8") as file:
    json.dump(zone_categories, file, ensure_ascii=False, indent=2)

preprocessing_metadata = {
    "model_type": "lightgbm_direct_multi_horizon",
    "prediction_strategy": "direct_non_recursive",
    "target_column": TARGET_COLUMN,
    "zone_column": ZONE_COLUMN,
    "issue_datetime_column": ISSUE_DATETIME_COLUMN,
    "target_datetime_column": TARGET_DATETIME_COLUMN,
    "horizon_column": HORIZON_COLUMN,
    "minimum_forecast_horizon": MIN_FORECAST_HORIZON,
    "maximum_forecast_horizon": MAX_FORECAST_HORIZON,
    ...
    "train_last_issue_datetime": str(train_data[ISSUE_DATETIME_COLUMN].max()),
    "train_last_target_datetime": str(train_data[TARGET_DATETIME_COLUMN].max()),
    "validation_first_issue_datetime": str(validation_data[ISSUE_DATETIME_COLUMN].min()),
    "validation_last_target_datetime": str(validation_data[TARGET_DATETIME_COLUMN].max()),
    "test_first_issue_datetime": str(test_data[ISSUE_DATETIME_COLUMN].min()),
}

metadata_path = os.path.join(ARTIFACT_DIRECTORY, "preprocessing_metadata.json")
with open(metadata_path, "w", encoding="utf-8") as file:
    json.dump(preprocessing_metadata, file, ensure_ascii=False, indent=2)

print(f"Artefacts créés dans : {ARTIFACT_DIRECTORY}")
...
with mlflow.start_run(
    run_name="lgbm_direct_multi_horizon_quantile_24h"
) as run:
    mlflow.log_params({
        "model_type": "LightGBM",
        "prediction_strategy": "direct_non_recursive",
    ...
    mlflow.log_metrics({
        ...
        "train_validation_bias": train_validation_metrics["bias"],
        ...
        "baseline_validation_wape": baseline_validation_metrics["wape"],
        "selected_validation_wape": float(chosen_result["wape"]),
    ...
    for _, row in metrics_by_forecast_day.iterrows():
        forecast_day = int(row["forecast_day"])
        mlflow.log_metrics({
            f"test_day_{forecast_day}_mae": float(row["mae"]),
            f"test_day_{forecast_day}_rmse": float(row["rmse"]),
            f"test_day_{forecast_day}_mape": float(row["mape"]),
            f"test_day_{forecast_day}_wape": float(row["wape"]),
            f"test_day_{forecast_day}_bias": float(row["bias"]),
        })

    mlflow.set_tags({
        "project": "energy_forecast",
        "forecast_type": "zonal_multi_horizon",
        "forecast_horizon": "1h_to_24h",
        "forecast_days": "1",
        "prediction_strategy": "direct_non_recursive",
        "data_table": GOLD_TABLE,
        "split_strategy": "walk_forward_backtesting",
        "feature_selection": "lightgbm_gain_validation_wape",
        "quantile_forecasting": "P10_P50_P90",
        "reconciliation": "bottom_up",
    })

    mlflow.log_artifacts(ARTIFACT_DIRECTORY, artifact_path="analysis")

    input_example_native = X_train_validation.head(5).copy()
    input_example_mlflow = prepare_mlflow_example(input_example_native)
    signature_predictions = final_model.predict(input_example_native)
    signature = infer_signature(input_example_mlflow, signature_predictions)
...
reload_test_sample = X_test.head(10).copy()
original_sample_predictions = final_model.predict(reload_test_sample)
reloaded_sample_predictions = reloaded_model.predict(reload_test_sample)

if not np.allclose(original_sample_predictions, reloaded_sample_predictions, rtol=1e-8, atol=1e-8):
    raise AssertionError("Les prédictions du modèle rechargé diffèrent du modèle initial.")

print("Test de rechargement MLflow réussi.")
...
print(f"Horizons                : H+{MIN_FORECAST_HORIZON} à H+{MAX_FORECAST_HORIZON}")
print(f"Stratégie               : directe, non récursive")
...
print(f"Biais test              : {test_metrics['bias']:.3f} MW")
...
for _, row in metrics_by_forecast_day.iterrows():
    print(f"J+{int(row['forecast_day'])} | MAE={row['mae']:.2f} MW | WAPE={row['wape']:.3f}% | Biais={row['bias']:.2f} MW")

print("\nTop 20 des features :")
for index, row in final_importance.head(20).iterrows():
    print(f"{index + 1:3d}. {row['feature']:50s} gain={row['gain_pct']:7.3f}%")

print("\nEntraînement multi-horizon 24h terminé avec succès.")
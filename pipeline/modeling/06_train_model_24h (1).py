...
# une table jamais créée par aucun script (04_build_features.py écrit dans
# `ml_features_gold_24h`). Corrigé pour pointer vers la vraie table. Le filtre
# sur forecast_horizon_hours == 24 plus bas reste inoffensif (la colonne y est
# toujours égale à 24 dans cette table).
GOLD_TABLE = f"{CATALOG}.{SCHEMA}.ml_features_gold_24h"
...
# NOTE (amélioration 2026-09-01) : recherche d'hyperparamètres Optuna, menée
# après la sélection des features sur le split train/validation (le test
# reste isolé). RUN_OPTUNA_SEARCH permet de la désactiver facilement.
...
def tune_hyperparameters_with_optuna(
    ...
    print(
        ...
        f"{len(train_frame):,} lignes d'entraînement et "
        f"{len(validation_frame):,} lignes de validation..."
    )
...
# MAGIC %md
# MAGIC ## 12bis. Recherche d'hyperparamètres (Optuna)
# MAGIC
# MAGIC Les paramètres LightGBM utilisés jusqu'ici (`num_leaves=31`,
# MAGIC `learning_rate=0.03`, etc.) sont des valeurs par défaut raisonnables,
...
# MAGIC split train/validation (le test reste isolé jusqu'à la section 14).
...
# Avec les hyperparamètres retenus, on redétermine le nombre optimal d'arbres
# par early stopping sur le split train/validation : un taux d'apprentissage
# différent change la vitesse de convergence et donc le nombre d'itérations
# utiles.
...
# la recherche Optuna (section 12bis) plutôt que de valeurs fixes.
final_params = {
    "objective": "regression_l1",
...
# DBTITLE 1,Analyse evals_result_ — courbe MAE validation
# --- Recharge les données et réentraîne avec capture de evals_result_ ---
# Objectif : reconstituer la courbe de MAE validation pour voir si elle
# était encore en baisse nette à l'itération 2997, ou quasi plate.
...
RUN_ID = "eff7fe63af414c369bba66d6a1dfda3e"

# 1. Charger la config pour le nom de la table Gold.
PROJECT_ROOT = "/Workspace/Users/n.jouglet23@gmail.com/energy_forecast_clean"
...
# 2. Charger les features sélectionnées depuis l'artefact MLflow.
features_uri = mlflow.artifacts.download_artifacts(
    run_id=RUN_ID, artifact_path="analysis/selected_features.json"
)
with open(features_uri) as f:
...
pd_data = spark_df.toPandas()
print(f"Lignes chargées : {len(pd_data):,}")

# 4. Préparation des types (identique au notebook).
for col in selected_features:
    if col == "zone":
        pd_data["zone"] = (
            pd_data["zone"].astype("string").fillna("UNKNOWN").astype("category")
        )
    elif pd.api.types.is_bool_dtype(pd_data[col]):
        pd_data[col] = pd_data[col].astype("int8")
    else:
        pd_data[col] = pd.to_numeric(pd_data[col], errors="coerce")

pd_data[TARGET_COLUMN] = pd.to_numeric(pd_data[TARGET_COLUMN], errors="coerce")
pd_data = pd_data[pd_data[TARGET_COLUMN].notna()].copy()
pd_data = pd_data.sort_values([DATETIME_COLUMN, "zone"]).reset_index(drop=True)

# 5. Split temporel strict (70/15/15).
unique_dts = np.sort(pd_data[DATETIME_COLUMN].dropna().unique())
train_end_idx = int(len(unique_dts) * 0.70)
val_end_idx = int(len(unique_dts) * 0.85)

train_end_dt = unique_dts[train_end_idx - 1]
val_start_dt = unique_dts[train_end_idx]
val_end_dt = unique_dts[val_end_idx - 1]
test_start_dt = unique_dts[val_end_idx]

train_data = pd_data[pd_data[DATETIME_COLUMN] <= train_end_dt].copy()
val_data = pd_data[
    (pd_data[DATETIME_COLUMN] >= val_start_dt)
    & (pd_data[DATETIME_COLUMN] <= val_end_dt)
].copy()

print(f"Train      : {len(train_data):,} lignes")
...
cat_features = [
    c for c in selected_features if str(X_train[c].dtype) == "category"
]

# 6. Entraînement A : hyperparamètres par défaut (lr=0.03, n_estimators=3000)
#    -> reproduit exactement le modèle de sélection de features.
print("\n--- Modèle A : params par défaut (lr=0.03, n_est=3000) ---")
...
# 7. Entraînement B : hyperparamètres Optuna (lr=0.01971, n_estimators=5000)
#    -> voit si le modèle tuné bénéficie aussi de plus d'itérations.
print("\n--- Modèle B : params Optuna (lr=0.01971, n_est=5000) ---")
model_b = lgb.LGBMRegressor(
    objective="regression_l1",
    n_estimators=5000,
    learning_rate=0.01971320798861417,
    num_leaves=215,
    max_depth=-1,
    min_child_samples=186,
    subsample=0.9996823661696368,
    subsample_freq=1,
    colsample_bytree=0.632974386622073,
    reg_alpha=0.8660924771653854,
    reg_lambda=0.004200724364816628,
    random_state=42,
    n_jobs=-1,
    verbosity=-1,
)
...
# 8. Extraction des courbes MAE validation.
evals_a = model_a.evals_result_["valid_0"]["l1"]  # MAE = l1 pour regression_l1
evals_b = model_b.evals_result_["valid_0"]["l1"]

print(f"\nCourbe A : {len(evals_a)} itérations")
print(f"Courbe B : {len(evals_b)} itérations")

# COMMAND ----------

# DBTITLE 1,Analyse pente MAE near iter 2997
# 9. Analyse de la pente du MAE validation près de l'itération 2997.

evals_a = model_a.evals_result_["valid_0"]["l1"]
evals_b = model_b.evals_result_["valid_0"]["l1"]

# --- Tableau récapitulatif : MAE à différents paliers ---
print("=" * 80)
print("MAE VALIDATION À DIFFÉRENTS PALIERS")
print("=" * 80)
print(f"{'Itération':>10} | {'MAE Modèle A (lr=0.03)':>24} | {'MAE Modèle B (lr=0.020)':>24}")
print("-" * 80)

checkpoints = [100, 500, 1000, 1500, 2000, 2500, 2800, 2900, 2950, 2997, 3000, 3500, 4000, 4500, 5000]
for cp in checkpoints:
    mae_a = f"{evals_a[cp - 1]:.4f}" if cp <= len(evals_a) else "—"
    mae_b = f"{evals_b[cp - 1]:.4f}" if cp <= len(evals_b) else "—"
    marker = " ← best_iter A" if cp == 3000 else (" ← best_iter B" if cp == 5000 else "")
    print(f"{cp:>10} | {mae_a:>24} | {mae_b:>24}{marker}")

# --- Calcul des deltas sur les dernières centaines d'itérations ---
print("\n" + "=" * 80)
print("VARIATION DU MAE SUR DES FENÊTRES GLISSANTES (Modèle A, lr=0.03)")
print("=" * 80)
windows_a = [(2500, 2600), (2600, 2700), (2700, 2800), (2800, 2900), (2900, 3000), (2950, 3000)]
for start, end in windows_a:
    if end <= len(evals_a):
        delta = evals_a[end - 1] - evals_a[start - 1]
        pct = (delta / evals_a[start - 1]) * 100
        print(f"  Iter {start}-{end:>4} : ΔMAE = {delta:+.4f} MW ({pct:+.3f} %)")

print("\n" + "=" * 80)
print("VARIATION DU MAE SUR DES FENÊTRES GLISSANTES (Modèle B, lr=0.020)")
print("=" * 80)
windows_b = [(2500, 2600), (2600, 2700), (2700, 2800), (2800, 2900), (2900, 3000),
            (3000, 3500), (3500, 4000), (4000, 4500), (4500, 5000)]
for start, end in windows_b:
    if end <= len(evals_b):
        delta = evals_b[end - 1] - evals_b[start - 1]
        pct = (delta / evals_b[start - 1]) * 100
        print(f"  Iter {start}-{end:>4} : ΔMAE = {delta:+.4f} MW ({pct:+.3f} %)")

# --- Graphique : courbes complètes + zoom sur les 500 dernières itérations ---
fig, axes = plt.subplots(1, 2, figsize=(18, 6))

# Vue globale
ax = axes[0]
ax.plot(range(1, len(evals_a) + 1), evals_a, label=f"Modèle A (lr=0.03, n_est=3000)", alpha=0.8)
ax.plot(range(1, len(evals_b) + 1), evals_b, label=f"Modèle B (lr=0.020, n_est=5000)", alpha=0.8)
ax.axvline(2997, color="red", linestyle="--", alpha=0.5, label="Iter 2997")
ax.set_xlabel("Itération")
ax.set_ylabel("MAE validation (MW)")
ax.set_title("Courbe MAE validation — vue globale")
ax.legend()

# Zoom sur les 500 dernières itérations de A (2500-3000) et équivalent pour B
ax = axes[1]
zoom_start_a = 2500
ax.plot(range(zoom_start_a, len(evals_a) + 1), evals_a[zoom_start_a - 1:],
        label="Modèle A (lr=0.03)", marker=".", markersize=2, linewidth=0.8)
zoom_b_start = 2500
ax.plot(range(zoom_b_start, len(evals_b) + 1), evals_b[zoom_b_start - 1:],
        label="Modèle B (lr=0.020)", marker=".", markersize=2, linewidth=0.8)
ax.axvline(2997, color="red", linestyle="--", alpha=0.5, label="Iter 2997")
ax.axvline(3000, color="orange", linestyle=":", alpha=0.5, label="Budget A (3000)")
ax.set_xlabel("Itération")
ax.set_ylabel("MAE validation (MW)")
ax.set_title("Zoom : itérations 2500+ — la baisse est-elle nette ou plate ?")
ax.legend()
...
# --- Verdict ---
delta_last_100_a = evals_a[2999] - evals_a[2899]
delta_last_500_a = evals_a[2999] - evals_a[2499]
delta_last_100_b = evals_b[4999] - evals_b[4899]
mae_min_b = min(evals_b)
mae_min_b_iter = evals_b.index(mae_min_b) + 1

print("\n" + "=" * 80)
print("VERDICT")
print("=" * 80)
print(f"Modèle A (lr=0.03, budget 3000) : best_iteration = {model_a.best_iteration_}")
print(f"  ΔMAE sur les 100 dernières itérations (2900→3000) : {delta_last_100_a:+.4f} MW")
print(f"  ΔMAE sur les 500 dernières itérations (2500→3000) : {delta_last_500_a:+.4f} MW")
print(f"  -> {'Baisse encore NETTE' if abs(delta_last_100_a) > 0.1 else 'Quasi PLAT'} sur les 100 dernières itérations")
print()
print(f"Modèle B (lr=0.020, budget 5000) : best_iteration = {model_b.best_iteration_}")
print(f"  ΔMAE sur les 100 dernières itérations (4900→5000) : {delta_last_100_b:+.4f} MW")
print(f"  MAE minimum atteint à l'itération {mae_min_b_iter} : {mae_min_b:.4f} MW")
print(f"  -> L'early stopping n'a JAMAIS déclenché sur aucun des deux modèles")

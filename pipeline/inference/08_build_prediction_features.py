...
def download_zone_weather_combined(
    ...
    prediction_end : pd.Timestamp
        Fin de la fenêtre (ancre 08h + 24h).
    ...
    # ============================================================
    # PARTIE 1 : HEURES PASSÉES (archive_start → now)
    # ============================================================
    # On récupère 388h AVANT prediction_start pour calculer les features de demande
    ...
    if archive_start < now:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Filtrer pour la fenêtre [archive_start, now[
                historical_df = historical_df[
                    (historical_df["target_datetime"] >= archive_start) &
...
# ============================================================
# CALCUL DE LA FENÊTRE DE PRÉDICTION
# ============================================================

# Ancre de prédiction : dernière heure 08h passée
forecast_anchor = (
    pd.Timestamp.now(tz=TIMEZONE)
    .tz_localize(None)
    .replace(hour=8, minute=0, second=0, microsecond=0)
)
if pd.Timestamp.now(tz=TIMEZONE).hour < 8:
    forecast_anchor -= pd.Timedelta(days=1)

prediction_start = forecast_anchor
prediction_end = forecast_anchor + pd.Timedelta(hours=25)

print("\n" + "=" * 80)
print("FENÊTRE DE PRÉDICTION")
print("=" * 80)
print(f"Maintenant              : {pd.Timestamp.now(tz=TIMEZONE).tz_localize(None)}")
print(f"Ancre 08h (dernière)    : {forecast_anchor}")
print(f"Début de prédiction     : {prediction_start}")
print(f"Fin de prédiction       : {prediction_end}")
print(f"Durée                   : 25 heures (08h → 08h inclus)")
...
for zone_key, zone_config in WEATHER_ZONES.items():
    ...
    try:
        zone_forecast = download_zone_weather_combined(
            zone_key=zone_key,
...
if not forecast_dfs:
    print("\nERREURS DE TÉLÉCHARGEMENT PAR ZONE :")
    for zone_key, error_msg in download_errors.items():
        print(f"  {zone_key}: {error_msg}")
    raise RuntimeError(
...
# (on garde TOUTES les prévisions disponibles, le filtrage pour la fenêtre
# de prédiction se fera plus tard)
...
# ============================================================
# FENETRE DE PREDICTION : DERNIÈRE ancre 08h passée -> 08h+24h
# ============================================================
# Pour un modèle itératif lancé aujourd'hui à 9h :
# - On prédit pour AUJOURD'HUI 08h → DEMAIN 08h (pas demain → après-demain)
#
# Logique :
# - Si exécuté AVANT 08h : prédire pour HIER 08h → AUJOURD'HUI 08h
# - Si exécuté APRÈS 08h : prédire pour AUJOURD'HUI 08h → DEMAIN 08h
#
# Le modèle itératif utilise les prévisions de maintenant jusqu'à l'ancre+24h.

current_time = pd.Timestamp.now(tz=TIMEZONE).tz_localize(None)

# Calculer la DERNIÈRE ancre 08h passée
prediction_start = current_time.replace(
    hour=8, minute=0, second=0, microsecond=0
)

# Si on est AVANT 08h aujourd'hui, la dernière ancre était hier
...
if weather_forecast_24h.empty:
    raise RuntimeError(
        f"Aucune donnée météo disponible pour la fenêtre de prédiction "
        f"{prediction_start} → {prediction_end}. "
        f"Heure actuelle : {current_time}"
    )

print(
    f"Fenêtre de prévision : "
    f"{prediction_start} -> {prediction_end}"
)

# ============================================================
# PRÉPARATION DES DONNÉES MÉTÉO HISTORIQUES
# ============================================================
# Extraire les données météo historiques depuis weather_forecast_24h
# (déjà téléchargées par download_zone_weather_combined avec 388h d'historique)

print("\n" + "=" * 80)
print("EXTRACTION DES DONNÉES MÉTÉO HISTORIQUES")
print("=" * 80)
...
# Renommer pour le format historique
weather_history = weather_history.rename(columns={
    "target_datetime": "datetime",
    "weather_target_temperature_c": "temperature_c",
    "weather_target_relative_humidity_pct": "relative_humidity_pct",
    "weather_target_dew_point_c": "dew_point_c",
    "weather_target_precipitation": "precipitation_mm",
    "weather_target_wind_speed_kmh": "wind_speed_kmh",
    "weather_target_wind_gusts_10m": "wind_gusts_kmh",
    "weather_target_cloud_cover": "cloud_cover_pct",
    "weather_target_surface_pressure": "surface_pressure_hpa",
})

# Convertir zone en string pour l'agrégation Ontario
weather_history["zone"] = weather_history["zone"].astype(str)
...
# ============================================================
# SAUVEGARDE DES FEATURES DANS UNITY CATALOG
# ============================================================
...
print(f"  ✅ {len(weather_forecast_24h.columns)} colonnes (toutes les features assemblées)")

# 3. Métadonnées de référence (ref_time, prediction_start, prediction_end)
...
print(f"     → Historique brut IESO (340h) pour prédiction itérative")
print(f"     → {len(demand_history):,} lignes")
print(f"  2. {FEATURE_TABLE}")
print(f"     → Toutes les features assemblées (météo + demande + historique météo)")
...
print("\nVous pouvez maintenant exécuter 09_batch_prediction_24h.py indépendamment.")

...
exec(
    open(
        f'{PROJECT_ROOT}/pipeline/inference/08_build_prediction_features.py'
    ).read()
)

# ============================================================
# UTILISER LA FENETRE CALCULEE PAR 08_build_prediction_features.py
# ============================================================

# Les variables prediction_start et prediction_end ont déjà été calculées
# par le script 08_build_prediction_features.py (dernière ancre 08h -> 08h+24h)
forecast_start = prediction_start
forecast_end = prediction_end

print(
    f"Fenêtre de prédiction : {forecast_start} -> {forecast_end}"
)
print(f"Lignes météo disponibles : {len(weather_forecast_24h)}")

if weather_forecast_24h.empty:
    raise ValueError(
        f"Aucune donnée météo disponible pour la fenêtre {forecast_start} -> {forecast_end}. "
        "Vérifiez que le script 08_build_prediction_features.py a bien récupéré les prévisions météo."
    )
